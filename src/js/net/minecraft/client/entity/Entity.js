...
export default class Entity {
  ...
  setPosition(x, y, z) {
    ...
    this.boundingBox = new BoundingBox(x - width, y, z - width, x + width, y + this.height, z + width)
  }
  ...
  moveCollide(targetX, targetY, targetZ) {
    ...
    if (this.onGround && this.isSneaking()) {
      for (
        ;
        targetX !== 0.0 && this.world.getCollisionBoxes(this.boundingBox.offset(targetX, -this.stepHeight, 0.0)).length === 0;
      ...
      for (
        ;
        targetZ !== 0.0 && this.world.getCollisionBoxes(this.boundingBox.offset(0.0, -this.stepHeight, targetZ)).length === 0;
      ...
      for (
        ...
        targetX !== 0.0 &&
        targetZ !== 0.0 &&
        this.world.getCollisionBoxes(this.boundingBox.offset(targetX, -this.stepHeight, targetZ)).length === 0;
        originalTargetZ = targetZ
      ) {
        if (targetX < 0.05 && targetX >= -0.05) {
          targetX = 0.0
