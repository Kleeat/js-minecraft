...
export default class CaveGenerator extends Generator {
  ...
  generateCaveAtBlock(
    ...
    for (; progress < distance; progress++) {
      ...
      if (
        distanceToCenterX * distanceToCenterX + distanceToCenterY * distanceToCenterY - progressInvert * progressInvert >
        shiftedAmplitude * shiftedAmplitude
      ) {
        return
      }
      ...
      for (let x = Math.floor(distanceToCenterX); !isWaterCave && x < layerX; x++) {
        for (let z = Math.floor(progressInvert); !isWaterCave && z < layerZ; z++) {
          for (let y = layerY + 1; !isWaterCave && y >= distanceToCenterY - 1; y--) {
            ...
            if (typeId === BlockRegistry.WATER.getId() || typeId === BlockRegistry.WATER.getId()) {
              // TODO one of them WATER MOVING
              isWaterCave = true
            }

            if (
              y !== distanceToCenterY - 1 &&
              x !== distanceToCenterX &&
              x !== layerX - 1 &&
              z !== progressInvert &&
              z !== layerZ - 1
          ...
          for (let y = layerY - 1; y >= distanceToCenterY; y--) {
            ...
            if (offsetY > -0.7 && offsetX * offsetX + offsetY * offsetY + offsetZ * offsetZ < 1.0) {
              ...
              if (
                ...
                typeId === BlockRegistry.DIRT.getId() ||
                typeId === BlockRegistry.GRASS.getId()
              ) {
                if (y < 10) {
                  // Lava cave
