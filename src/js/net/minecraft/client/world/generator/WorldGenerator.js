...
export default class WorldGenerator extends Generator {
  ...
  populateChunk(chunkX, chunkZ) {
    ...
    let amount = Math.floor(
      (this.populationNoiseGenerator.perlin(absoluteX * 0.5, absoluteY * 0.5) / 8 + this.random.nextDouble() * 4 + 4) / 3,
    )
    if (amount < 0) {
      amount = 0
    }
    ...
    amount = Math.floor(
      (this.populationNoiseGenerator.perlin(absoluteX * 0.5, absoluteY * 0.5) / 8 + this.random.nextDouble() * 4 + 10) / 2,
    )
    console.log(amount)
    if (amount < 0) {
      amount = 0
  ...
  generateTerrainNoise(noiseX, noiseY, noiseZ, width, height, depth) {
    ...
    let terrainNoise3 = this.terrainGenerator3.generateNoiseOctaves(
      noiseX,
      noiseY,
      noiseZ,
      width,
    ...
    let terrainNoise4 = this.terrainGenerator4.generateNoiseOctaves(
      noiseX,
      noiseY,
      noiseZ,
      width,
    ...
    let terrainNoise5 = this.terrainGenerator5.generateNoiseOctaves(
      noiseX,
      noiseY,
      noiseZ,
      width,
